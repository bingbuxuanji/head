...
{
  /* SPI GPIO/clock already configured by MX_SPI2_Init() in spi.c */
  /* W25Q128 works with CPOL=Low, CPHA=1Edge (SPI Mode 0) */

  __HAL_SPI_ENABLE(&hspi2);

  /* 清空上电后 SPI RX 缓冲区残留，避免后续读取整体偏移 */
  while (__HAL_SPI_GET_FLAG(&hspi2, SPI_FLAG_RXNE)) {
    volatile uint32_t __attribute__((unused)) dummy = READ_REG(hspi2.Instance->DR);
  }
}
