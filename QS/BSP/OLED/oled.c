 ...
 */
/*
 * Convert HZK16 row-major 16x16 font to SSD1306 column-major format.
 * HZK16: 32 bytes, row-major (rows 0-15, each row = left8 + right8, MSB=leftmost pixel)
 * OLED:  32 bytes, column-major (cols 0-15 of upper half + cols 0-15 of lower half, LSB=top)
 */
static void HZK16_To_OLED(uint8_t *src, uint8_t *dst)
{
	uint8_t col, row, byte_idx, bit_pos;
	uint8_t mask;
...
/**
 * @function: void OLED_ShowCHinese_GB(uint8_t x, uint8_t y, uint8_t gb_h, uint8_t gb_l, uint8_t Color_Turn)
 * @description: Display a 16x16 Chinese character from W25Q16 flash using GB2312 encoding
 * @param {uint8_t} x horizontal position (0~112)
 * @param {uint8_t} y page (0~6)
 * @param {uint8_t} gb_h GB2312 high byte (0xB0~0xF7)
 * @param {uint8_t} gb_l GB2312 low byte (0xA1~0xFE)
 * @param {uint8_t} Color_Turn 1=invert, 0=normal
 */
void OLED_ShowCHinese_GB(uint8_t x, uint8_t y, uint8_t gb_h, uint8_t gb_l, uint8_t Color_Turn)
{
	uint8_t hzk_buf[32];
	uint8_t oled_buf[32];
	uint16_t index;
	...
	index = GB2312_CodeToIndex(gb_h, gb_l);
	if (index == 0xFFFF)
	{
		for (t = 0; t < 32; t++) oled_buf[t] = 0x00;
	}
	else
	{
		GB2312_GetFontByIndex(index, hzk_buf);
		HZK16_To_OLED(hzk_buf, oled_buf);
...
/**
 * @function: void OLED_ShowString_GB2312(uint8_t x, uint8_t y, char *str, uint8_t Color_Turn)
 * @description: Display a GB2312-encoded Chinese string from W25Q16 flash
 * @param {uint8_t} x horizontal start position (0~112)
 * @param {uint8_t} y page (0~6)
 * @param {char*} str GB2312-encoded string (mixing ASCII and Chinese)
 * @param {uint8_t} Color_Turn 1=invert, 0=normal
 */
void OLED_ShowString_GB2312(uint8_t x, uint8_t y, char *str, uint8_t Color_Turn)
{
	uint8_t cx = x;
	uint8_t cy = y;
	...
	while (*str != '\0')
	{
		/* Check if it's a Chinese character (high byte >= 0xB0 for GB2312) */
		if ((uint8_t)*str >= 0xB0 && *(str + 1) != '\0')
		{
			if (cx > 112)
			{
				cx = 0;
			...
			OLED_ShowCHinese_GB(cx, cy, (uint8_t)*str, (uint8_t)*(str + 1), Color_Turn);
			cx += 16;
			str += 2;
		}
		else
		{
			/* ASCII character, use 8x16 font */
			if (cx > 120)
			{
				cx = 0;
				cy += 2;
			...
			OLED_ShowChar(cx, cy, (uint8_t)*str, 16, Color_Turn);
			cx += 8;
			str += 1;
		}
	}
...
/**
 * @function: void OLED_ShowChar_UTF8(uint8_t x, uint8_t y, const uint8_t *utf8_char, uint8_t Color_Turn)
 * @description: Display a single UTF-8 encoded Chinese character (16x16) from W25Q16 font
 * @param {uint8_t} x horizontal position (0~112)
 * @param {uint8_t} y page (0~6)
 * @param {const uint8_t*} utf8_char pointer to UTF-8 encoded character bytes
 * @param {uint8_t} Color_Turn 1=invert, 0=normal
 */
void OLED_ShowChar_UTF8(uint8_t x, uint8_t y, const uint8_t *utf8_char, uint8_t Color_Turn)
{
	uint16_t unicode;
	uint8_t  gb_h, gb_l;
	...
	if (UnicodeToGB2312(unicode, &gb_h, &gb_l) != 0)
		return;

	OLED_ShowCHinese_GB(x, y, gb_h, gb_l, Color_Turn);
}

/**
 * @function: void OLED_ShowString_UTF8(uint8_t x, uint8_t y, char *str, uint8_t Color_Turn)
 * @description: Display a UTF-8 string (mixing ASCII and Chinese) from W25Q16 font
 * @param {uint8_t} x horizontal start position (0~112)
 * @param {uint8_t} y page (0~6)
 * @param {char*} str UTF-8 encoded null-terminated string
 * @param {uint8_t} Color_Turn 1=invert, 0=normal
 */
void OLED_ShowString_UTF8(uint8_t x, uint8_t y, char *str, uint8_t Color_Turn)
{
	uint8_t cx = x;
	uint8_t cy = y;
	...
	while (*str != '\0')
	{
		...
		if (c < 0x80)
		{
			...
			OLED_ShowChar(cx, cy, c, 16, Color_Turn);
			cx += 8;
			str += 1;
		}
		else
		{
			...
			uint16_t unicode;
			uint8_t adv = UTF8_Decode((const uint8_t *)str, &unicode);

			if (adv == 0 || adv > 4)
			{
				str += 1;  /* skip invalid byte */
				continue;
			}
			...
			if (UnicodeToGB2312(unicode, &gb_h, &gb_l) == 0)
			{
				OLED_ShowCHinese_GB(cx, cy, gb_h, gb_l, Color_Turn);
			}
