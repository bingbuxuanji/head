...
本项目以 **Quectel EC800 模组** 为核心，运行 QuecPython（MicroPython），承担全部业务逻辑：语音 AI 对话、云端通信、骑行导航、MQTT 上报、MCP 工具服务。下位机 **STM32G431CB** 通过 UART 挂载，负责传感器采集、OLED 显示、摔倒检测，向 EC800 上报数据并接收显示指令。

**EC800 核心功能**：
- 离线语音唤醒（KWS）与语音活动检测（VAD）
- 云端 WebSocket 实时语音对话（Opus 编解码）
- 本地 TTS 语音合成 + 骑行导航（路段跟踪/偏航检测/重规划冷却）
- MCP 工具调用（音量/唤醒词/路线查询/导航启停/导航急停）
- MQTT 云平台集成（属性上报/事件上报）
- OTA 升级 + sys_bus 事件总线
- UART 指挥 G431：GPS 请求 `g`、导航/时间/心情下发 `t`、初始化握手 `i`

**G431 外设**：
- 传感器：MAX30102（心率）、BMP280（温度/气压）、MPU6050（六轴 IMU）、BH1750（光照）
- OLED 128×32：双行布局，UTF-8 中文字库（W25Q128 SPI Flash），导航滚动 + 时间/心率/心情
- 摔倒/碰撞检测：合加速度 > 3g 或角速度 > 300°/s 即时告警
- 测试模式：模拟 GPS 轨迹 + 模拟传感器数据
