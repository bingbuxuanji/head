# 智能头盔/语音助手项目 README（EC800 核心 + STM32G431 传感）

## 1. 项目概述

本项目以 **Quectel EC800 模组** 为核心，运行 QuecPython（MicroPython），承担全部业务逻辑。下位机 **STM32G431CB** 通过 UART 挂载，负责传感器采集与 OLED 显示，向 EC800 上报数据并接收显示/采集指令。两机通过串口协议（`b/g/i/m/s/t`）协作。

**EC800 支持**：
- 离线语音唤醒（KWS）与语音活动检测（VAD），最多 3 个唤醒词
- 云端 WebSocket 实时语音对话（Opus 编解码）
- 本地 TTS 语音合成（用于导航指令等离线播报）
- 充电管理与电源控制
- GPS/北斗定位数据解析与主动轮询
- 按键交互（音量调节、唤醒、开关机）
- 串口通信扩展（与 G431 双向：`g` GPS 请求、`t` 导航文字/时间/LLM心情、`i` 初始化握手）
- MCP 工具调用（音量控制、唤醒词改名、骑行路线查询、导航启停/急停）
- OTA 升级（获取 WebSocket 连接参数）
- 实时骑行导航（路段跟踪、偏航检测、到达提醒、重规划冷却）
- 高精度球面几何计算（点到折线最近距离、方位角等）
- sys_bus 事件总线（模块间解耦通信）
- MQTT 云平台集成（属性上报、事件上报）
- 6 路 LED 指示灯（网络状态、AI 对话、电源状态）

**G431 支持**：
- 多传感器采集：MAX30102（心率/血氧）、BMP280（温度/气压）、MPU6050（六轴 IMU）、BH1750（光照）
- OLED 128×32 双行布局：上行导航文字 UTF-8 滚动（W25Q128 字库），下行时间/心率/LLM 心情
- 摔倒/碰撞检测：合加速度 > 3g 或角速度 > 300°/s 即时告警
- 4 键输入 + 测试模式（默认开启，模拟 GPS 轨迹 + 模拟传感器数据）
...
- **关键点**：
  ...
  - 内置 2 秒超时保护：若 MCU 未在 2 秒内回复，自动清除 pending 标志防止死锁。
    - 同时承担每秒时间同步：`tHH:MM:SS[ mood]` 报文发往 G431 OLED 显示
  - UART 每收到 G431 数据也触发后备时间发送（`Massage._tick_cb`），防止 HTTP 阻塞时断更

**G431 主循环**（独立 MCU，不占 EC800 线程）：

G431 的 `def_main()` 在 `while(1)` 中运行，由 TIM2 硬件定时器驱动时序：
- 每轮：`sersor_data()` — MAX30102 FIFO 快排
- 每 10ms：`key_scan()` — 按键扫描，上报 `b1`~`b4`
- 每 100ms：`imu_read()` → `imu_fall_detect()` — 六轴 + 摔倒检测
- 每 500ms：`sersor_slow_read()` + `nav_scroll_update()` — BMP280 + 导航文字滚动
- 每 5s：`send_sensor_uart()` + `send_imu_uart()` — 定时上报（收到 EC800 `i` 指令后开启）
- 测试模式：GPS 每 5 分钟模拟推进，传感器每 5 秒模拟更新
...
本项目构建了 EC800（QuecPython）+ STM32G431（裸机）双 MCU 智能头盔系统。

- **EC800 核心**：AI 语音对话 + 骑行导航 + MQTT 上报 + MCP 工具服务，6 类线程 + UART Tick 回调 + 硬件定时器协同
- **G431 外设**：多传感器采集 + OLED 双行显示（导航滚动/时间/心率/LLM 心情）+ W25Q128 字库 + 摔倒检测
- **双机握手**：`i<hex>` 上报传感器就绪 + EC800 `i` 启动采集，支持 G431 热复位；所有 UART 消息 `\r\n` 分行防粘包
- **导航完备**：高德 API 路线规划 → polyline 解析 → 球面几何跟踪 → 偏航重规划（45s 冷却 + 失败 OLED 提示）
- **测试模式**：G431 默认开启，模拟 GPS 轨迹 + 传感器数据，无需真实 GPS/传感器即可调试全链路
